...
async function run() {
  ...
  const page2Res = await fetch('https://servicios.pami.org.ar/vademecum/views/consultaPublica/presentacion.zul', { headers: getHeaders });
  const page2Html = await page2Res.text();
  
  console.log("Presentacion page contains isologo1.jpg?", page2Html.includes('isologo1.jpg'));
  fs.writeFileSync('zkPresentacion.txt', page2Html);
}
run();
